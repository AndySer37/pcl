...
void cluster_pointcloud()
{
  ...
  for (int i = 0; i < cloud_filtered->points.size(); i++)
  {
    ...
    if (cloud_filtered->points[i].y <=2.5 && cloud_filtered->points[i].y >= 0 && cloud_filtered->points[i].x >= -1.5 && cloud_filtered->points[i].x <= 1.5)
    {
      hl_indices->indices.push_back(i);
    }
  }
  ...
  *cloud_filtered = *cloud_h;
  ...
  for (std::vector<pcl::PointIndices>::const_iterator it = cluster_indices.begin (); it != cluster_indices.end (); ++it)
  {
    ...
    for(int n = 0; n <= 6; n++)
    {
      ...
      normalEstimation_model.setRadiusSearch(0.15);
      ...
      std::cout << "instances: " << transformations.size() << std::endl;
      if(transformations.size() > 0)
      {
        ...
        if (max_instance < transformations.size())
        {
          max_instance = transformations.size();
          match_model_index = n;
        }
      }
    }
    ...
    if(is_match)
    {
      ...
      {
        if (match_model_index <= 3)
        {
          ob_pose.r = 1;
          std::cout<<"Found Buoy"<<std::endl;
        }
        else if (match_model_index <= 6)
        {
          ob_pose.r = 2;
          std::cout<<"Found Totem"<<std::endl;
        }
