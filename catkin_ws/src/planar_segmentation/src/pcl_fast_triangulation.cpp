...
void cbPointCloud(const sensor_msgs::PointCloud2::ConstPtr& cloud_msg)
{
    ...
    float max_range = 40, min_range = 0, dis;
    for (int i=0 ; i <  cloud_voxel->points.size() ; i++)
    {
        ...
        if (dis<min_range || dis > max_range || cloud_voxel->points[i].z > 1.5 || cloud_voxel->points[i].z < -0.5)
        {
    ...
    /*
    //Visual polygonmesh by pcl viewer
 	if (first_Set_Viewer)
	{
